...
class ToolManager:
    ...
    @classmethod
    def _disabled_secret_names(cls) -> set[str]:
        return {
            item.strip()
    ...
    def secrets_for_sandbox(
        ...
        """
        disabled_secrets = self._disabled_secret_names()
        out: list[SecretDef] = [
            secret
            for secret in self._INFRA_SECRETS
            if secret.name not in disabled_secrets
        ]
        for lt in self.tools.values():
            out.extend(lt.all_secrets)
        out.extend(
            secret
            for secret in self._harness_secrets_for(engine, auth_modes)
            if secret.name not in disabled_secrets
        )
        return out
    ...
    def collect_secrets(self) -> list[SecretDef]:
        ...
        for harness_set in self._HARNESS_SECRETS.values():
            out.extend(
                secret for secret in harness_set if secret.name not in disabled_secrets
            )
        return out
